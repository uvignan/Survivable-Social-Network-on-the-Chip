...
function validateUserNameAndPassword(sourceUser) {
...
if ($('#userName' + sourceUser).val().trim() != "") {
    if ($('#userName' + sourceUser).val().trim() == "") {
        $('#alert' + sourceUser).text('Please provide a username to continue');
        $('#alert' + sourceUser).show();
        return false;
    }


    if ($('#userName' + sourceUser).val().trim().length < 3) {
        $('#alert' + sourceUser).text('Please provide a username with atleast 3 characters');
        $('#alert' + sourceUser).show();
    ...
    re = /^\w+$/g;
    if (!re.test($('#userName' + sourceUser).val().trim())) {
        $('#alert' + sourceUser).text('Please do not include special characteristics');
        $('#alert' + sourceUser).show();
        $('#passport_alert').remove();
        return false;
    }
    ...
    for (var i = 0; i < res.length; ++i) {
        if (($('#userName' + sourceUser).val().trim() == res[i])) {
            $('#alert' + sourceUser).text("Please do not use this banned username");
            $('#alert' + sourceUser).show();
            $('#passport_alert').remove();
            return false;
        }
    ...
    if (($('#password' + sourceUser).val().trim() != "")) {

        if (($('#password' + sourceUser).val().trim() == "")) {
            $('#alert' + sourceUser).text("Please provide a password to continue");
            $('#alert' + sourceUser).show();
            $('#passport_alert').remove();
            return false;
        }
        ...
        if ($('#password' + sourceUser).val().trim().length < 4) {
            $('#alert' + sourceUser).text("Please enter password atleast 4 characters");
            $('#alert' + sourceUser).show();
            $('#passport_alert').remove();
            return false;
        }

        if ($('#password' + sourceUser).val().trim() == $('#userName' + sourceUser).val().trim()) {
            $('#alert' + sourceUser).text("Please enter password different from username");
            $('#alert' + sourceUser).show();
            $('#passport_alert').remove();
            return false;
        }
